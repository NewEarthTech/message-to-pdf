...
import { DOWNLOAD_ENDPOINT, PADDLE_ENV } from "@/lib/paddle-config"

// The second and last island. Checkout redirects here with
// ?_ptxn=<transaction_id>, which only exists at runtime, so the page is
// prerendered in its "no purchase found" state and this fills in on load.
//
// Gating model: server-verified entitlement. This component never holds a
// download URL. It links to the download gate with the transaction id, and that
// endpoint verifies the transaction against the Paddle API (real, paid, and for
// this product) before redirecting to a short-lived signed URL. The DMG has no
// public read path, so a visitor who lands here without a paid transaction gets
// a 403, not a file.
const linkClass =
    ...
    <>
      ...
      <div className="mt-10 rounded-2xl border border-[#93735a]/40 bg-[#403f3c] px-7 py-8 sm:px-9">
        <p className="text-[#f5f3ef]/75 text-sm leading-relaxed">
          Message to PDF for macOS. Universal (Apple Silicon and Intel), macOS 11 and later, signed
      ...
      <p className="mt-6 text-[#f5f3ef]/65 text-xs leading-relaxed">
        ...
        . By downloading you accept the GPL-3.0-or-later license shipped in the app.
      </p>
    </>
  )
}
