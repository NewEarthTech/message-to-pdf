...
Terraform for the marketing site at **message-to-pdf.com**: a private S3 bucket fronted by CloudFront (Origin Access Control) with a DNS-validated ACM certificate. The site is the Astro project in [`../../apps/www/`](../../apps/www/); `pnpm build` renders it to static HTML in `apps/www/dist/`, which is synced to the bucket.
...
Pushing to `main` with changes under `apps/www/**` or `packages/ui/**` runs `.github/workflows/deploy-www.yml`, which builds and syncs. To deploy by hand:

```sh
cd ../../apps/www && pnpm install && pnpm build
...
The build refuses to run without a matching `PUBLIC_PADDLE_ENV` / `PUBLIC_PADDLE_CLIENT_TOKEN` pair, so a sandbox token can never ship to production.
